...
from data.data_utils import mark_begin_end_sentence_in_text
...
class OpenQAPDataset(ABC, Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.with_extractive_loss:
            titles = [ctx['title'] for ctx in sample['ctxs'][:self.topk]]
            contexts = [ctx['text'] for ctx in sample['ctxs'][:self.topk]]
            extractive_info, contexts = self.mark_start_end_in_topk(answers, titles, contexts, question)
            item['extractive'] = extractive_info
        for ctx_index in range(self.topk):
            ctx = sample['ctxs'][ctx_index]
            text = ctx['text'] if contexts is None else contexts[ctx_index]
            title = ctx['title']
            input_texts.append(self.before_context_pattern.format(**{'question': question, 'title': title}) + text)
            # if contexts is not None:
            #     if ctx['text'] != contexts[ctx_index]:
            #         print("Original:", ctx['text'])
            #         print("New     :", contexts[ctx_index])
            #         print()
        ...
        if self.with_generative_loss:
            decoder_tokenized_output = self.tokenizer(text_target='<extra_id_0> ' + random.choice(answers),
                                                    padding="max_length",
                                                    truncation=True,
    ...
    def align_start_end_with_tokenizer(self, answers, title, text, question):
        start_positions_text = []
        end_positions_text = []
        before_context = self.before_context_pattern.format(**{'question': question, 'title': title})
        for answer in answers:
            start_positions, end_positions, new_text = mark_begin_end_sentence_in_text(answer, text)
            start_positions = [s + len(before_context) for s in start_positions]
            end_positions = [e + len(before_context) for e in end_positions]
            start_positions_text.extend(start_positions)
            end_positions_text.extend(end_positions)
        text = new_text
        ...
        return t5_tokenized_starts, t5_tokenized_ends, eos_index, text
    
    def mark_start_end_in_topk(self, answers, titles, contexts, question):
        ...
        local_have_answer = []
        new_contexts = []
        for title, text in zip(titles, contexts):
            current_starts, current_ends, eos_index, new_context = self.align_start_end_with_tokenizer(answers, title, text, question)
