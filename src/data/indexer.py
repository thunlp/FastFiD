...
class OpenRetreivalDataStore(object):
    ...
    def __init__(self, args, load_from_path=True):
    ...
    def load_from_file(self):
        ...
        logger.info("> Unpickling BlockData: {}".format(self.embedding_path))
        state_dict = pickle.load(open(self.embedding_path, 'rb'))
        logger.info(">> Finished unpickling BlockData")
    ...
    def add_block_data(self, row_id, block_embeds, allow_overwrite=False):
        for idx, embed in zip(row_id, block_embeds):
            ...
            self.embed_data[idx] = np.float32(embed)
    ...
    def save_shard(self):
        ...
        logger.warning("Save shard {} to {}/{}.pkl".format(self.rank, self.temp_dir_name, self.rank))
    ...
    def merge_shards_and_save(self):
        ...
        logger.info("Begin merge different shards")
...
class FaissMIPSIndex(object):
    ...
    def _set_mips_index(self):
        ...
        if self.use_gpu:
            ...
            logger.info(">> Initialized index on GPU")
        else:
            self.mips_index = faiss.IndexIDMap(cpu_index)
            logger.info(">> Initialized index on CPU")
    ...
    def add_embed_data(self, all_embed_data):
        ...
        logger.info(">>> Finished adding block data to index")
...
class DistributedBruteForceIndex(object):
    ...
    def _set_mips_index(self):
        """Create a Faiss Flat index with inner product as the metric to search against"""
        logger.info("> Building Brute Force MIPS index")
    ...
    def add_embed_data(self, all_embed_data):
        ...
        # the embeddings have to be entered in as float32 even though the math internally is done with float16.
        block_embeds = torch.from_numpy(np.array(block_embeds, dtype=np.float32))
        ...
        logger.info(">>> Finished adding block data to GPU index")
    ...
    def search_mips_index(self, query_embeds, top_k, reconstruct=True):
        ...
        C = torch.zeros(query_embeds.shape[0], self.num_rows, dtype=torch.float32, device="cuda")
