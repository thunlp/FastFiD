...
class OpenQAPDataset(ABC, Dataset):
    def __init__(self, args, path, split, tokenizer):
        ...
        if format_type == 0:
            ...
            raise ValueError(f"format_type {format_type} is not implemented.")
    ...
    def collate_fn(batch):
        ...
        for i in range(batch_size):
            ...
            new_batch[context_ids][i] = new_batch[context_ids][i] + [pad_id] * (max_len - current_len)
            new_batch[context_attention_mask][i] = [1] * current_len + [0] * (max_len - current_len)
        ...
        for i in range(batch_size):
            ...
            new_batch["prompt_mask"][i] = new_batch["prompt_mask"][i] + [0] * (max_len - current_len)
